...
public final class BookSummaryService {
    ...
    private static final int RETRY_PROMPT_CHARS = 3200;
    ...
    public SummaryResult generateSummaryFromBookFile(String filePath, SummaryStyle style) {
        ...
        if (!hfConfiguredChecker.getAsBoolean()) {
            ...
            return SummaryResult.success(
                    fallback,
                    selectedStyle.label() + " summary generated locally. Review and edit before finalizing.");
        }
        ...
        try {
            ...
            if (summary == null || summary.isBlank()) {
                ...
                if (!retrySummary.isBlank()) {
                    return SummaryResult.success(
                            retrySummary,
                            selectedStyle.label() + " summary generated by LLM. Review and edit before finalizing.");
                }
                String fallback = buildLocalExtractiveSummary(normalizedPreview, selectedStyle);
                if (!fallback.isBlank()) {
                    return SummaryResult.success(
    ...
    static String toOllamaBody(String input, SummaryStyle style) {
        SummaryStyle selectedStyle = style == null ? SummaryStyle.MEDIUM : style;
        ...
        return "{"
                + "\"model\":\"" + escapeJson(AppConfig.OLLAMA_MODEL) + "\","
    ...
    private static final class HttpApiCaller implements Function<RequestPayload, ApiResponse> {
        ...
        public ApiResponse apply(RequestPayload payload) {
            ApiResponse primary = AppConfig.preferOllamaSummary()
                    ? tryOllama(payload)
                    : tryHuggingFace(payload);
            if (primary.ok) {
                return primary;
            }

            ApiResponse secondary = AppConfig.preferOllamaSummary()
                    ? tryHuggingFace(payload)
                    : tryOllama(payload);
            if (secondary.ok) {
                return secondary;
            ...
            return new ApiResponse(
                    false,
                    "remote unavailable",
                    "{\"error\":\"" + escapeJson(primary.reason + "; " + secondary.reason) + "\"}"
            );
        }

        private ApiResponse tryOllama(RequestPayload payload) {
            if (!AppConfig.isOllamaConfigured()) {
                return new ApiResponse(false, "ollama not configured", "{\"error\":\"Ollama not configured\"}");
            }
            String endpoint = AppConfig.getOllamaGenerateEndpoint();
            String body = toOllamaBody(payload.input(), payload.style());
            return sendJsonRequest(endpoint, body, false);
        }

        private ApiResponse tryHuggingFace(RequestPayload payload) {
            if (!AppConfig.isHfConfigured()) {
                return new ApiResponse(false, "hf not configured", "{\"error\":\"HF not configured\"}");
            }
            String endpoint = AppConfig.getHfInferenceEndpoint();
            String body = toInferenceBody(payload.input(), payload.style());
            return sendJsonRequest(endpoint, body, true);
        }

        private ApiResponse sendJsonRequest(String endpoint, String body, boolean useAuthHeader) {
