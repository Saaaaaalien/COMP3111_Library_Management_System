...
- **Task 1 (Student/Staff):** Registration, login, available books list, borrow book, My Borrowed Books (view and return). Borrow limit (5 per user), due date (14 days), Quick Review (first few pages), Read Summary pop-up. 15‑minute inactivity timer; lockout after 5 failed login attempts. **Session restore:** on startup, if `data/session.json` is present, the app restores the last saved route for Student/Staff—either pre-account screens (Welcome, Student/Staff portal, login, or register with `userId` 0) or, after login, Available Books, My Borrows, profile, notifications, reading history, book request, or the PDF reader (borrow, book, page, zoom). If that snapshot is invalid or unsafe (for example missing borrow or wrong role), the user is sent to **Available Books** instead, with a short dialog explaining full or partial recovery. Logout clears the session file. For a manual smoke check, use Welcome → Student/Staff Portal → Login → open a few drawer destinations, quit without logging out, and relaunch; optional **Crash test** on Welcome (visible when dev mode is on, the default) saves `WELCOME` and terminates the JVM to mimic a kill.
